...
- AI 服务通过 Provider 接口隔离，后续可以替换不同模型或服务。
...
具体采样率取决于 AI 服务要求。若服务支持 24k PCM，就用 24k；否则用 16k。
...
优先方案 A。如果实时模型事件适配受阻，则退回方案 B，先牺牲一点延迟，保证 Demo 稳定。
...
### 12.3 AudioWorklet 与降级

优先使用 `AudioWorklet`。

若需要降级，用 `ScriptProcessorNode` 先跑通链路。它已经被标记为旧 API，但可用于 MVP 验证。

建议先用 `ScriptProcessorNode` 验证链路，再替换成 AudioWorklet。
...
- 实现 preload 的基础 API。
- 实现麦克风采集。
- 实现 SessionManager 骨架。
- 实现一个 Provider mock，先模拟字幕输出。
- 接入真实 AI Provider。
...
以下问题会影响实现细节，需要在实现前确认：

1. AI 服务最终使用哪一家？是否已有 API Key？
2. 演示机器是否固定 Windows？
3. 是否必须支持系统音频直采，还是麦克风采集可作为 MVP？
4. 默认语言候选是否只保留“自动识别、英语、日语、韩语、简体中文”？
5. 是否需要在演示中展示双语对照，还是只展示目标语言字幕？
